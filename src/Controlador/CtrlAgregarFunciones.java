...
public class CtrlAgregarFunciones implements ActionListener {
    ...
    public void obtenerDatos(int indexCombobox) throws Exception {
        ...
        this.funcion.setHoraPresentacion(
                new Time(timeFormater.parse(vista.getComboBoxHora().getSelectedItem().toString()).getTime()));
        this.funcion.setObra(this.obras.get(indexCombobox - 1));

    }

    // Verifica que no haya más de dos obras en el dia o que no interfiera una con
    // otra
        ...
        if (funciones.isEmpty()) {
            ...
            if (!funciones.get(0).getHoraPresentacion().equals(this.funcion.getHoraPresentacion())) {
                if (funciones.get(0).getHoraPresentacion().equals(new Time(18, 0, 0))
                    ...
                    disponible = true;
                } else if (this.funcion.getHoraPresentacion().equals(new Time(18, 0, 0))
                        && funcion.getObra().getDuracion() < 150) {
                    disponible = true;
                }
            } else {
                throw new ExcepcionHorarioNoDisponible(
                        "El horario no se encuentra disponible, por favor seleccione otro");
            }
        }
        if (disponible == false)
